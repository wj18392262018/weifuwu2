...
<div class="register py-container ">
    ...
    <div class="registerArea" id="registerApp">
        <h3>注册新用户<span class="go">我有账号，去<a href="${pageContext.request.contextPath}/login.jsp" target="_blank">登陆</a></span></h3>
        <div class="info" style="width: 650px">
            <form class="sui-form form-horizontal" method="post"  action="${pageContext.request.contextPath}/category/addUser">
                ...
                <div class="control-group">
                    ...
                    <div class="controls btn-reg">
                        <%--<a class="sui-btn btn-block btn-xlarge btn-danger" href="javascript:void(0)" target="_blank"
                           @click.stop="submit"
                        ></a>--%><button class="sui-btn btn-block btn-xlarge btn-danger" type="submit">完成注册</button>
                    </div>
                </div>
            </form>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/pages/register.js"></script>
        ...
        methods: {
            ...
            submit() {
                this.$validator.validateAll().then(d => {
                    if (d) {
                        // 校验通过，提交表单
                        leyou.http.post("/category/register", leyou.stringify(this.user))
                            .then(resp => {
                            if (resp.status === 201) {
                            // 注册成功
                            alert("注册成功,即将跳转到登录页！");
                            setTimeout(() => window.location = '${pageContext.request.contextPath}/login.html', 2000);
                        }
                    })
                    .catch(() => alert("注册失败！"))
                    }
